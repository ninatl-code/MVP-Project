...
export default function ReservationsPrestataire() {
  ...
  // Fonction pour marquer automatiquement les commandes comme livrées
  useEffect(() => {
    const checkDeliveredOrders = async () => {
      ...
      const now = new Date()
      const ordersToDeliver = reservations.filter(r => {
        const reservationDate = new Date(r.date)
        // Marquer comme livrée 1 jour après la date prévue si expédiée
        const deliveryDate = new Date(reservationDate.getTime() + 24 * 60 * 60 * 1000)
        return r.status === 'shipped' && deliveryDate < now
      })

      if (ordersToDeliver.length > 0) {
        const updatePromises = ordersToDeliver.map(r => 
          supabase
            .from('reservations')
              ...
              <div className="flex gap-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-indigo-400">
                    {reservations.filter(r => r.status === 'shipped').length}
                  </div>
                  <div className="text-slate-300 text-sm">Expédiées</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-400">
                    {reservations.filter(r => r.status === 'delivered').length}
                  </div>
                  <div className="text-slate-300 text-sm">Livrées</div>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-1">
                {filteredReservations.length > 0 ? (
                  ...
                  <div className="bg-white rounded-lg shadow-sm p-8 text-center">
                    ...
                    <h2 className="text-lg font-medium text-slate-600 mb-2">Aucune commande trouvée</h2>
