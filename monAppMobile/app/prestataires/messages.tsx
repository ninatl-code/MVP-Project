...
export default function MessagesPrestataire() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    const { error } = await supabase.from('messages').insert({ conversation_id: selectedConv.id, sender_id: user.id, receiver_id: selectedConv.client_id, contenu: messageInput, objet: 'Message' });
    if (!error) {
          ...
          <FlatList data={conversations} keyExtractor={(item) => item.id} contentContainerStyle={styles.listContent} ItemSeparatorComponent={() => <View style={styles.separator} />} renderItem={({ item }) => (
            <TouchableOpacity style={styles.conversationItem} onPress={() => handleSelectConv(item)}>
              <View style={styles.avatarContainer}>
                {item.client.photos?.[0] ? <Image source={{ uri: item.client.photos[0] }} style={styles.avatar} /> : <View style={[styles.avatar, styles.avatarPlaceholder]}><Text style={styles.avatarText}>{getInitials(item.client.nom)}</Text></View>}
                {!item.lu && <View style={styles.unreadDot} />}
              </View>
              <View style={styles.conversationContent}>
                <View style={styles.conversationHeader}>
                  <Text style={[styles.clientNom, !item.lu && styles.boldText]}>{item.client.nom}</Text>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }} keyboardVerticalOffset={90}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <View style={styles.smallAvatar}>{selectedConv.client.photos?.[0] ? <Image source={{ uri: selectedConv.client.photos[0] }} style={styles.smallAvatarImage} /> : <Text style={styles.smallAvatarText}>{getInitials(selectedConv.client.nom)}</Text>}</View>
            <Text style={styles.headerName}>{selectedConv.client.nom}</Text>
          </View>
